...
library(openxlsx)
library(ggplot2)
library(lemon) # not necessary as loaded when used, but nice to know here...
...
for (year in years) {
  for (out in outcomes) {
    ...
    survfit_sc_dis <- survfit(Surv(age_tstop, event) ~ 1, data = get(paste0("studybase_", out, year)), 
                          subset = get(paste0("sc_", out)) == 1, weights = get(paste0("w_sc_", out)), cluster = xnr)
    ...
    survfit_sc_any <- survfit(Surv(age_tstop, event) ~ 1, data = get(paste0("studybase_", out, year)), 
                          subset = sc_any == 1, weights = w_sc_any, cluster = xnr)
...
# Risk estimates at ages 18,25,30,40
...
for (year in years) {
  for (out in outcomes){
    ...
    res_full <- assign(paste0("res_cuminc_full_",out,year),res_times(get(paste0("survfit_full_",out,year)),out,year,c(18,25,30), "full"))
    res_sc <- assign(paste0("res_cuminc_sc_",out,year),res_times(get(paste0("survfit_sc_",out,year)),out,year, c(18,25,30), "sc"))
    res_sc_dis <- assign(paste0("res_cuminc_sc_",out,"_",out,year),res_times(get(paste0("survfit_sc_",out,"_",out,year)),out,year, c(18,25,30), "sc_dis"))
    res_sc_all <- assign(paste0("res_cuminc_sc_any_",out,year),res_times(get(paste0("survfit_sc_any_",out,year)),out,year, c(18,25,30), "sc_any"))
    all_results_risk_2015 <- append(all_results_risk_2015, list(bind_rows(res_full,res_sc,res_sc_dis,res_sc_all)))
    cat("\n") # blank line
  }
}
comb_res_risk_2015 <- bind_rows(all_results_risk_2015)
comb_res_risk_2015
write.xlsx(comb_res_risk_2015,"risks_FU2015_aff_epi.xlsx",overwrite=TRUE)

# risk by ages 18,25,30,40 for 2021
all_results_risk_2021 <- list()
years <- c(2021) 
outcomes <- c("affek","epi")
models <- list()
for (year in years) {
  for (out in outcomes){
    cat("Year:", year," , ", "Outcome:", out,  "\n")
    res_full <- assign(paste0("res_cuminc_full_",out,year),res_times(get(paste0("survfit_full_",out,year)),out,year,c(18,25,30,40), "full"))
    res_sc <- assign(paste0("res_cuminc_sc_",out,year),res_times(get(paste0("survfit_sc_",out,year)),out,year, c(18,25,30,40), "sc"))
    res_sc_dis <- assign(paste0("res_cuminc_sc_",out,"_",out,year),res_times(get(paste0("survfit_sc_",out,"_",out,year)),out,year, c(18,25,30,40), "sc_dis"))
    res_sc_all <- assign(paste0("res_cuminc_sc_any_",out,year),res_times(get(paste0("survfit_sc_any_",out,year)),out,year, c(18,25,30,40), "sc_any"))
    all_results_risk_2021 <- append(all_results_risk_2021, list(bind_rows(res_full,res_sc,res_sc_dis,res_sc_all)))
    cat("\n") # blank line
  }
}
comb_res_risk_2021 <- bind_rows(all_results_risk_2021)
comb_res_risk_2021
write.xlsx(comb_res_risk_2021,"risks_FU2021_aff_epi.xlsx",overwrite=TRUE)
...
plot3 <- pred_risk_aff_epi %>%
  ...
  theme(strip.placement = "outside",
        ...
        #axis.text.y.right = element_text(angle = 90, hjust = c(0,0,1)),

...
ASD <- studybase %>%
  select(pnr,fdato.x,asd_1d) %>%
  filter(!is.na(asd_1d))
# ASD times
asd_times <- ASD %>%
  mutate(asd_time = as.numeric((asd_1d - fdato.x)/365.24),
         asd_exp = 1) %>%
  select(pnr, asd_time, asd_exp)
...
for (year in years) {
  for (out in outcomes){
    ...
    assign(paste0("base1_",out,year),get(paste0("studybase_",out,year)) %>% 
             select(pnr, event, age_tstart, age_tstop, fdato.x, kqn.x, 
    ...
    base2 <- tmerge(base1, base1, id=pnr, tstart = 1, tstop = age_tstop, event=event(surv_time,event))
    ...
    m3 <- coxph(Surv(tstart,tstop,event) ~ exposure + cluster(pnr), data = get(paste0("df_asd_",out)), subset=get(paste0("sc_",out))==1, weights = get(paste0("w_sc_",out)))
    m4 <- coxph(Surv(tstart,tstop,event) ~ exposure + cluster(pnr), data = get(paste0("df_asd_",out)), subset=sc_any==1, weights = w_sc_any)
